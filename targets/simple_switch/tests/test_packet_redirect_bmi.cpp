...
// struct pcap {
// 	struct Packet { timeval time_stamp; std::vector<u_char> data; pcap* pcap_object; };
//   std::string get_name();
...
class Ports {
public:
  static std::string generate_name_from_number(int port_number) { return "i" + std::to_string(port_number); }
  static int get_number_from_name(const std::string port_name) { assert(port_name[0] == 'i'); return atoi(&port_name[1]); }
  //NOTE: Port numbers are 0-based, so number can be directly used as an index.
  static void add_args_for_n_ports(std::vector<std::string>& args_to_modify, int n_ports) {
    for (int i = 0; i < n_ports; ++i) {
      args_to_modify.push_back("-i");
      args_to_modify.push_back(std::to_string(i)+"@"+generate_name_from_number(i));
    }
  }
  void update_ports(int n_ports) {
    ports_.clear();
    for (int i = 0; i < n_ports; ++i)
      ports_.push_back(pcap_mock::get_pcap_object(generate_name_from_number(i)));
  }
  pcap* get_port_from_number(int port_number) { return ports_.at(port_number); }
  int get_number_from_port(pcap* port) { return get_number_from_name(pcap_mock::get_name(port)); }

  void send(int port, const char* data, size_t data_size) {
    using namespace std::literals::chrono_literals;
    std::this_thread::sleep_for(100ms);
    pcap_mock::simulate_packets_received(get_port_from_number(port), std::vector<std::vector<u_char>>{std::vector<u_char>(data, data+data_size)});
  }
...
// class PcapMockPacketInject {
// public:
//   PcapMockPacketInject(Ports& ports) :
//     ports_(ports)
//   {}
//   void send(int port, const char* data, size_t data_size) {
//     interfaces_.at(port)->simulate_packets_received(std::vector<std::vector<u_char>>{std::vector<u_char>(data, data+data_size)});
//   }

// private:
//   std::vector<pcap*>& interfaces_;
// };
}  // namespace
...
class SimpleSwitch_PacketRedirectP4 : public ::testing::Test {
 protected:
  ...
  static void SetUpTestCase() {
    ...
    // for (int i=1; i<=n_interfaces; ++i) { auto i_str = std::to_string(i); args.push_back("-i"); args.push_back(i_str+"@i"+i_str); }
    std::vector<char*> argv; for (auto&& a : args) argv.push_back(a.data());
    test_switch->init_from_command_line_options(argv.size(), argv.data(), nullptr, nullptr);
...
Ports SimpleSwitch_PacketRedirectP4::packet_inject;
...
TEST_F(SimpleSwitch_PacketRedirectP4, Baseline) {
  ...
  // interfaces[port_in]->simulate_packets_received(std::vector<std::vector<u_char>>{std::vector<u_char>(pkt, pkt+4)});
  // pcap::Packet recv_packet = interfaces[port_out]->get_sent_packet();
  // unsigned char* recv_buffer = recv_packet.data.data(); // [kMaxBufSize];
