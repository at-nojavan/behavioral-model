...
public:
  ...
  static int get_number_from_name(const std::string port_name) { 
    assert(port_name[0] == 'i'); return atoi(&port_name[1]); 
  }
  ...
  static void add_args_for_n_ports(std::vector<std::string>& args_to_modify, 
    ...
    for (int i = 0; i < n_ports; ++i) {
      ...
      args_to_modify.push_back(std::to_string(i)+"@"+
                              generate_name_from_number(i));
    }
  }
  void update_ports(int n_ports) {
    ports_.clear();
    for (int i = 0; i < n_ports; ++i)
      ports_.push_back(pcap_mock::get_pcap_object(
        generate_name_from_number(i)));
  }
  ...
  void send(int port, const char* data, size_t data_size) {
    pcap_mock::simulate_packets_received(get_port_from_number(port), 
      std::vector<std::vector<u_char>>{std::vector<u_char>
      (data, data+data_size)});
  }
...
class SimpleSwitch_PacketRedirectP4 : public ::testing::Test {
 protected:
  ...
  static void SetUpTestCase() {
    ...
    // for (int i=1; i<=n_interfaces; ++i) { auto i_str = std::to_string(i); 
    args.push_back("-i"); args.push_back(i_str+"@i"+i_str); }
    std::vector<char*> argv; for (auto&& a : args) argv.push_back(a.data());
    test_switch->init_from_command_line_options(argv.size(), 
                                                argv.data(), nullptr, nullptr);
